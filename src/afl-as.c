...
static void add_instrumentation(void) {
  ...
  u8 instr_ok = 0, skip_csect = 0, skip_next_label = 0, skip_att = 0,
  ...
  while (fgets(line, MAX_LINE, inf)) {
    ...
    /* Detect syntax changes, as could happen with hand-written assembly.
       Skip AT&T blocks, resume instrumentation when back to Intel. */

    if (strstr(line, ".intel_syntax")) { skip_att = 0; }
    if (strstr(line, ".att_syntax")) { skip_att = 1; }
    ...
    if (skip_att || skip_app || skip_csect || !instr_ok || line[0] == '#' ||
        line[0] == ' ') {
  ...
  if (!be_quiet) {
    ...
    if (!ins_lines) {
      ...
      OKF("Instrumented %u locations (64-bit, %s mode, ratio %u%%).", ins_lines,
          modeline, inst_ratio);
